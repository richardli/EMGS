...
for(itr in 1:Nsim){
	for(index in 1:dim(config)[1]){
		...
		save(sim, file = paste("../data/sim/graphsim", n, p, graph, itr, "gaussian.rda", sep="_"))
	}
}
...
set.seed(itr * 123456)
load( paste("../data/sim/graphsim", n, p, graph, itr, "gaussian.rda", sep="_"))
X <- sim$data
...
lambda <- 1
Prec <- sim$K  
out <- EMGS(as.matrix(X), v0s, v1, lambda, a, b, 0.001, 1e4, verbose = TRUE, copula = FALSE)
out.npn = huge(X, method = "glasso", nlambda=40,lambda.min.ratio = 0.4)
...
save(fit, file = paste("../data/fit/graphfit", n, p, graph, itr, "gaussian.rda", sep="_"))
}