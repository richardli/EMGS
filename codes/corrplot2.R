...
corrwraper <- function(mat, title, group, lim){
    ...
    corrplot2(mat[order, order],  tl.pos = 'n', title = title, mar = c(0,0,3,0), method="color", bg = "gray70", na.label.col = "gray70", is.corr=F, cl.lim = lim)
    if(!is.null(group)){
        corrRect(table(as.character(group)))
    }
}

## A modified version to avoid checking ylim when NA exists
